...
if (typeof(L.CartoDBLayer) === "undefined") {
   ...
   *  infowindow    -     If you want to see infowindows when click in a 
   *                                geometry (opcional - default = false)
   *  auto_bound    -     Let cartodb auto-bound-zoom in the map 
  ...
  L.CartoDBLayer = function (params) {
    ...
    if (this.params.auto_bound)
      autoBound(this.params);  // Bounds? CartoDB does it.
    
    if (this.params.infowindow) {
      addWaxCartoDBTiles(this.params)
    } else {
      addSimpleCartoDBTiles(this.params); // Always add cartodb tiles, 
                                          // simple or with wax.
    }

    // this is obstrusive, we are paying to use cartodb and should be able
    // to choose if we want to see this or not, on mobile devices
    // it looks awful
    if (this.params.show_cartodb_logo)
      addCartodb();
  ...
  function autoBound(params) {
      ...
      reqwest({
        url:'http://'+params.user_name+'.cartodb.com/api/v1/sql/?q='+
          escape('select ST_Extent(the_geom) from '+ params.table_name),
        type: 'jsonp',
        jsonpCallback: 'callback',
        success: function(result) {
          if (result.rows[0].st_extent!=null) {
            var coordinates = result.rows[0].st_extent.replace('BOX(','')
                                                      .replace(')','')
                                                      .split(',');
    ...
    function addSimpleCartoDBTiles(params) {
      ...
      var tile_style = (params.tile_style) ? 
        encodeURIComponent(params.tile_style.replace(
          /\{\{table_name\}\}/g,params.table_name)) : '',
      query = encodeURIComponent(
        params.query.replace(/\{\{table_name\}\}/g,params.table_name));

      // Add the cartodb tiles
      var cartodb_url = 'http://' + params.user_name + 
        '.cartodb.com/tiles/' + 
        params.table_name + 
        '/{z}/{x}/{y}.png?sql=' + 
        query +'&style=' + tile_style, 
    ...
    // convert a TouchEvent to  MouseEvent if required
  ...
  function addWaxCartoDBTiles(params) {
      ...
      callbacks: {
        ...
        click: function(feature, div, op3, evt) {
          // convert TouchEvent to MouseEvent if required
          e = touchHandler(evt); 
          latlng = params.map.mouseEventToLatLng(e);
      ...
      params.interaction = 
        wax.leaf.interaction(params.map, params.tilejson, params.waxOptions);
